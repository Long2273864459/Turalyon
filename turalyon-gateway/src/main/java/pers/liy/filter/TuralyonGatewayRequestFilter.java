...
import pers.liy.properties.TuralyonGatewayProperties;
...
@Slf4j
@Component
public class TuralyonGatewayRequestFilter implements GlobalFilter {

    @Resource
    private TuralyonGatewayProperties properties;
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        byte[] token = Base64Utils.encode((TuralyonConstant.ZUUL_TOKEN_VALUE).getBytes());
        ServerHttpRequest build = request.mutate().header(TuralyonConstant.ZUUL_TOKEN_HEADER, new String(token)).build();
