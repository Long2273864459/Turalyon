...
import pers.liy.exception.TuralyonException;
...
public class UserController {
    ...
    public CloudResponse userList(QueryRequest queryRequest, SystemUser user) {
        Map<String, Object> dataTable = TuralyonUtil.getDataTable(userService.findUserDetail(user, queryRequest));
        return new CloudResponse().data(dataTable);
    }

    @PostMapping
    @PreAuthorize("hasAnyAuthority('user:add')")
    public void addUser(@Valid SystemUser user, BindingResult bindingResult) throws TuralyonException {
        try {
            this.userService.createUser(user);
        } catch (Exception e) {
            String message = "新增用户失败";
            log.error(message, e);
            throw new TuralyonException(message);
        }
    }

    @PutMapping
    @PreAuthorize("hasAnyAuthority('user:update')")
    public void updateUser(@Valid SystemUser user) throws TuralyonException {
        try {
    ...
    public void deleteUsers(@NotBlank(message = "{required}") @PathVariable String userIds) throws TuralyonException {
        try {
